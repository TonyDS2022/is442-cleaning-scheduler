...
=======
    ...
    public ResponseEntity<String> addContract(@RequestBody Contract contract) {
        try {
            clientContractService.addContract(contract);
...
>>>>>>> Stashed changes
    ...
    // localhost:8080/api/v0.1/contract/set-up-client-contract/
    @PostMapping("/set-up-client-contract/")
    public ResponseEntity<String> setUpClientContract(@RequestBody Contract contract) {
        try {
            clientContractService.setUpClientContract(contract);
            return ResponseEntity.status(HttpStatus.ACCEPTED).body("Contract added successfully, with its associated cleaning sessions and shifts.");
        } catch (IllegalArgumentException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Unable to add contract.");
        }
    }
